...
export default function SignInPage() {
  ...
  const handleNextStep = (e: React.FormEvent) => {
    ...
    else if (step === 2 && password) handleSignIn(e);
  };
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Welcome back to DebugMind AI!");
      router.replace("/dashboard");
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-[#050505] flex flex-col justify-center items-center px-6 relative overflow-hidden font-inter">
      ...
      >
        ...
        <div className="hidden lg:flex flex-col p-16 bg-gradient-to-br from-blue-600/10 to-transparent border-r border-white/5 relative overflow-hidden">
          ...
          <div className="mt-auto space-y-12 relative z-10">
             <div className="space-y-4">
               <h2 className="text-4xl font-black text-white leading-tight">The developer's <br /> <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">new best friend.</span></h2>
        ...
        <div className="p-8 md:p-16 flex flex-col justify-center relative bg-black/20">
          <div className="mb-10 text-center lg:text-left">
             <h3 className="text-3xl font-black text-white mb-2 tracking-tight">Login</h3>
             <p className="text-zinc-500 font-bold uppercase text-[10px] tracking-[0.2em]">Start your session to begin debugging</p>
          </div>

          <form onSubmit={handleNextStep} className="space-y-8">
            <div className="space-y-6">
              <div className="space-y-3">
                <label className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em] ml-1">Email Identity</label>
                <div className="relative group">
                  <Mail className={`absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 transition-colors ${step === 2 ? "text-blue-500" : "text-zinc-700"}`} />
                  <input
                    type="email"
                    required
                    readOnly={step === 2}
                    ...
                    className={cn(
                      "w-full bg-[#0a0a0a] border border-white/5 rounded-2xl py-5 pl-14 text-white font-bold focus:outline-none transition-all placeholder:text-zinc-800",
                      step === 1 ? "pr-6 focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/5" : "pr-24 text-zinc-400 border-blue-500/20 bg-blue-500/[0.03]"
                    )}
                  />
                  {step === 2 && !loading && (
                    <button 
                      type="button" 
                      onClick={() => setStep(1)} 
                      className="absolute right-4 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-blue-500/10 border border-blue-500/20 rounded-lg text-[10px] font-black text-blue-500 uppercase hover:bg-blue-500 hover:text-white transition-all shadow-lg"
                    >
                      Change
                    </button>
                  )}
                ...
                {step === 2 && (
                  ...
                  >
                    <div className="flex justify-between items-center ml-1">
                      ...
                      <Link href="#" className="text-[10px] text-blue-500 hover:text-blue-400 font-black uppercase tracking-widest transition-colors">Recover</Link>
                    </div>
                    <div className="relative group">
            ...
            >
              <div className="relative z-10 flex items-center justify-center gap-3">
                {loading ? (
                  ...
                  <>
                    {step === 1 ? "Next Step" : "Access Workspace"} 
          ...
          <div className="relative mt-12 mb-8">
            ...
            <div className="relative flex justify-center text-[10px] uppercase">
              <span className="bg-[#0c0c0e] px-4 text-zinc-700 font-black tracking-[0.3em]">Network Access</span>
            </div>
          </div>
          ...
          <p className="mt-auto pt-16 text-center text-zinc-600 text-[10px] font-black uppercase tracking-[0.1em]">
            New to the grid? <Link href="/signup" className="text-blue-500 hover:text-blue-400 hover:underline transition-all ml-1">Establish Credentials</Link>
          </p>
        </div>
      </motion.div>
